...
PI_L2 unsigned char *imgBuff;
static unsigned char *buff_inverting;
...
void cluster_inverting(void* args){
    ...
    // 确保没有内核会尝试处理图像范围之外的像素
    if(upper_bound > total ) upper_bound = total; 
    // loop over the area assigned to the core

    // 每个内核处理指定范围内的像素。循环遍历分配给该内核的像素索引，通过从 255 中减去像素值来反转每个像素。结果存储在 resBuffer 中
...
void cluster_delegate(void *arg){
    ...
    pi_cl_team_fork(8, cluster_inverting, arg);
}
//==================================KAI ADDED==================================
...
void camera_task(void *parameters){
    ...
    imgBuff = pmsis_l2_malloc(captureSize);
    if (imgBuff == NULL){
    ...
    //==================================KAI ADDED==================================
    buff_inverting = (unsigned char *)pmsis_l2_malloc(captureSize);
    if (buff_inverting == NULL){
        return;
    }
    //==================================KAI ADDED==================================
        ...
        if (wifiClientConnected == 1){
            ...
            if (streamerMode == JPEG_ENCODING){
                ...
                plp_example_kernel_instance_i32 args = {
                    .srcBuffer = (char*)imgBuff,        // 指向原图像数据的缓冲区
                    .resBuffer = (char*)buff_inverting,        // 指向处理后图像数据的缓冲区
                    .width = CAM_WIDTH, 
                    .height = CAM_HEIGHT, 
                    .nPE = 8,    // 处理器内核的数量，用于并行处理
                ...
                processTime = time_after - time_before;
                ...
                start = xTaskGetTickCount();
                // Send image
                sendBufferViaCPX(&txp, buff_inverting, imgSize);

                transferTime = xTaskGetTickCount() - start;
            }
            // #ifdef OUTPUT_PROFILING_DATA
            cpxPrintToConsole(LOG_TO_CRTP, "capture=%dms, processing=%d us (%d bytes), transfer=%d ms\n",
                              captureTime, processTime, imgSize, transferTime);
            // #endif
        }
