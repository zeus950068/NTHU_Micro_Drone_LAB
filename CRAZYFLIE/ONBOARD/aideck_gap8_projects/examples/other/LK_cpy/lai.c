...
//-----------------------------------------------------------------------------
  ...
  pi_buffer_init(&buffer2, PI_BUFFER_TYPE_L2, imgBuff2);
   ...
   pi_buffer_init(&buffer4, PI_BUFFER_TYPE_L2, imgBuff4);
  ...
  pi_buffer_set_format(&buffer4, CAM_WIDTH, CAM_HEIGHT, 1, PI_BUFFER_FORMAT_GRAY);
      ...
      if (captureState == 0)
            {
                ...
                //memcpy(imgBuff3 ,imgBuff2 ,CAM_WIDTH*CAM_HEIGHT) ;
               
               // memcpy(imgBuff3 ,imgBuff2 ,CAM_WIDTH*CAM_HEIGHT) ;
               /*
                if(captureState1 == 1)
                {
                calculate_flow_grid(imgBuff2, imgBuff, CAM_WIDTH, CAM_HEIGHT, grid, 4, 8); // Using the same half_patch_size and iterations as before
                }
          else
          {
          captureState1 = 1 ;
          ...
          */
            ...
            else if (captureState == 1)
            {
                ...
                calculate_flow_grid(imgBuff , imgBuff2, CAM_WIDTH, CAM_HEIGHT, grid, 4, 8); // Using the same half_patch_size and iterations as before
...
// #ifdef OUTPUT_PROFILING_DATA
      cpxPrintToConsole(LOG_TO_CRTP, "capture_opt=%dms, imgSize=(%d bytes), transfer=%d ms, alltime=%d ms\n",
                        captureTime, imgSize, transferTime, alltime);
// #endif
    }
    else
    {
